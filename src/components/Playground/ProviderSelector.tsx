...
export function ProviderSelector({ selected, onChange, userApiKeys, singleSelect = false }: ProviderSelectorProps) {
  ...
  const checkServerHealth = async () => {
    ...
    try {
      ...
      // If the health check automatically switched to browser mode, update our state
      const currentMode = localStorage.getItem('modelshift-connection-mode') || 'server';
      if (currentMode !== connectionMode) {
        setConnectionMode(currentMode);
      }
    } catch (error) {
  ...
  const getServerStatusMessage = () => {
    ...
    if (proxyHealth.available) {
      if (proxyHealth.errors.length > 0) {
        // Check if the error indicates we switched to browser mode
        const switchedToBrowser = proxyHealth.errors.some(error => 
          error.includes('Switched to direct browser mode')
        );
        if (switchedToBrowser) {
          return 'Using direct browser mode';
        }
  ...
  const getServerStatusColor = () => {
    ...
    if (proxyHealth.available) {
      if (proxyHealth.errors.length > 0) {
        // Check if we switched to browser mode
        const switchedToBrowser = proxyHealth.errors.some(error => 
          error.includes('Switched to direct browser mode')
        );
        if (switchedToBrowser) {
          return 'bg-green-500'; // Green because browser mode is working
        }
  ...
  return (
    <div className="space-y-4">
      ...
      {proxyHealth?.errors && proxyHealth.errors.length > 0 && (
